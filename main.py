...
#*Вычисление координат Senkoface*#
print('Координата: %s' % str(max_loc))   
print('Ебаната: %s' % max_val)


ebanata = 0.36                          #вписыываем сюда положение
if max_val >= ebanata:
    ...
    cv.rectangle(screenshot, top_left, bottom_right, 
                    color=(0, 100, 0), thickness=4, lineType=cv.LINE_4)

  
    cv.imwrite('result.jpg', screenshot) #результат поиска

else:
    print('Нассы себе на лицо!')
